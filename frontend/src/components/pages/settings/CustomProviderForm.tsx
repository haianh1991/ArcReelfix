...
function useMediaTypeOptions() {
  const { t } = useTranslation();
  return [
    { value: "text", label: t("provider.type_text") },
    { value: "image", label: t("provider.type_image") },
    { value: "video", label: t("provider.type_video") },
  ] as const;
}
...
function usePriceLabel() {
  const { t } = useTranslation();
  return (mediaType: MediaType): { input: string; output: string } => {
    if (mediaType === "video") return { input: t("provider.unit_sec"), output: "" };
    if (mediaType === "image") return { input: t("provider.unit_pic"), output: "" };
    return { input: t("provider.unit_min"), output: t("provider.unit_mout") };
  };
}
...
export function CustomProviderForm({ existing, onSaved, onCancel }: CustomProviderFormProps) {
  ...
  const handleDiscover = useCallback(async () => {
    if (!baseUrl) {
      showError(t("provider.err_base_url"));
      return;
    }
    if (!apiKey) {
      showError(t("provider.err_api_key"));
      return;
    }
    setDiscovering(true);
    try {
      ...
      showError(e instanceof Error ? e.message : t("provider.err_models"));
    } finally {
      setDiscovering(false);
    }
  }, [apiFormat, baseUrl, apiKey, isEdit, showError, t]);
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      setTestResult({ success: false, message: e instanceof Error ? e.message : t("provider.err_test") });
    } finally {
      setTesting(false);
    }
  ...
  const handleSave = useCallback(async () => {
    // Validation
    if (!displayName.trim()) {
      showError(t("provider.err_name_req"));
      return;
    }
    if (!baseUrl.trim()) {
      showError(t("provider.err_url_req"));
      return;
    }
    if (!isEdit && !apiKey.trim()) {
      showError(t("provider.err_key_req"));
      return;
    }
    const enabledModels = models.filter((m) => m.is_enabled);
    if (enabledModels.length === 0) {
      showError(t("provider.err_model_req"));
      return;
    }
    const emptyId = enabledModels.find((m) => !m.model_id.trim());
    if (emptyId) {
      showError(t("provider.err_model_id"));
      return;
    }
    setSaving(true);
    try {
      ...
      showError(e instanceof Error ? e.message : t("provider.err_save"));
    } finally {
      setSaving(false);
    }
  ...
  const priceLabelFn = usePriceLabel();
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <h3 className="mb-6 text-lg font-semibold text-gray-100">
        {isEdit ? t("provider.title_edit") : t("provider.title_add")}
      </h3>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="cp-name" className="mb-1.5 block text-sm text-gray-400">
            {t("provider.lbl_name")} <span className="text-red-400">*</span>
          </label>
          <input
            id="cp-name"
            type="text"
            ...
            placeholder={t("provider.ph_name")}
            className={inputCls}
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={isEdit ? existing?.api_key_masked ?? t("provider.ph_key_edit") : t("provider.ph_key_add")}
            ...
            <button
              ...
              aria-label={showApiKey ? t("provider.hide") : t("provider.show")}
            >
        ...
        {models.length > 0 && (
          <div>
            <div className="mb-2 flex items-center gap-3 text-sm text-gray-400">
              <span>{t("provider.model_list")}</span>
              {models.length > 1 && (
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  {filteredModels.every((m) => m.is_enabled) ? t("provider.deselect_all") : t("provider.select_all")}
                </button>
              )}
            </div>
            {models.length > 5 && (
              <div className="relative mb-2">
                ...
                <input
                  ...
                  placeholder={t("provider.search_ph")}
                  className="w-full rounded-lg border border-gray-900 py-1.5 pl-8 pr-3 text-xs text-gray-100 placeholder-gray-600 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            )}
            <div className="space-y-2">
              {filteredModels.map((m) => {
                const pl = priceLabelFn(m.media_type);
                return (
                  <div
                    key={m.key}
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <label className="flex cursor-pointer items-center gap-1.5">
                        <input
                          ...
                          aria-label={t("provider.enable_model")}
                        />
                      </label>
                      ...
                      <input
                        ...
                        aria-label={t("provider.model_id_lbl")}
                      ...
                      <select
                        ...
                        aria-label={t("provider.media_type")}
                        className={selectCls}
                      >
                      ...
                      <button
                        ...
                        aria-label={t("provider.del_model")}
                      >
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 pl-6 text-xs text-gray-500">
                      <select
                        ...
                        aria-label={t("provider.currency")}
                      ...
                      <input
                        ...
                        placeholder="0.00"
                        aria-label={t("provider.input_price")}
                      ...
                      {pl.output && (
                        <>
                          ...
                          <input
                            ...
                            placeholder="0.00"
                            aria-label={t("provider.output_price")}
                            className="w-16 rounded border border-gray-700 bg-gray-900 px-1.5 py-0.5 text-xs text-gray-300 placeholder-gray-600 focus-visible:border-indigo-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-indigo-500"
                          />
                          <span>{pl.output}</span>
                        </>
