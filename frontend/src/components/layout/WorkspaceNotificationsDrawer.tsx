        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <span className="rounded-full border border-white/8 bg-white/4 px-2.5 py-1 text-[11px] text-slate-300">
              {workspaceNotifications.length} {t("auto.notification")}
            </span>
            <span className="flex items-center gap-1.5 text-[11px] text-amber-100/85">
              <Sparkles className="h-3.5 w-3.5" />
              {t("auto.unread")} {unreadCount}
            </span>
          </div>
          <button
            type="button"
            ...
            aria-label={t("auto.close_notification_p")}
          >
            <X className="h-4 w-4" />
          </button>
        </div>
      ...
      <div className="max-h-[28rem] overflow-y-auto px-3 py-3">
        {workspaceNotifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 rounded-[1.1rem] border border-dashed border-white/10 bg-white/[0.03] px-6 py-12 text-center">
            ...
            <div>
              <p className="text-sm text-slate-200">{t("auto.there_are_currently_")}</p>
              <p className="mt-1 text-xs text-slate-500">
                {t("auto.project_refreshes_bu")}
              </p>
            </div>
          </div>
        ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] uppercase tracking-[0.18em] text-slate-400">
                          {item.read ? t("auto.read") : t("auto.new_notification")}
                        </span>
                      ...
                      <div className="mt-3 flex items-center justify-between gap-2">
                        {actionable ? (
                          ...
                          >
                            {t("auto.view_positioning")}
                            <ArrowUpRight className="h-3.5 w-3.5" />
                          </button>
                        ) : (
                          <span className="text-[11px] text-slate-500">{t("auto.notify_only")}</span>
                        )}
                        <button
                          type="button"
      ...
      {workspaceNotifications.length > 0 && (
        <div className="border-t border-white/8 px-4 py-2.5">
          <div className="flex items-center justify-between text-[11px] text-slate-500">
            <span>{t("auto.opening_a_panel_will")}</span>
            <span className="inline-flex items-center gap-1">
              <CheckCheck className="h-3.5 w-3.5" />
              {t("auto.temporary_session_re")}
            </span>
          </div>
        </div>
      )}
...
function formatNotificationTime(timestamp: number): string {
  ...
  if (diff < 60_000) return standaloneT("auto.just");
