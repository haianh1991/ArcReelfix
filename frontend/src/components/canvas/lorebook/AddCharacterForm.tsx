  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-200">{t("auto.add_role")}</h3>
        <button
          type="button"
          onClick={onCancel}
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">
            {t("auto.name")} <span className="text-red-400">*</span>
          </label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t("auto.character_name")}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">
            {t("auto.describe")} <span className="text-red-400">*</span>
          </label>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder={t("auto.description_of_the_c")}
            rows={3}
        ...
        <div>
          ...
          <input
            ...
            placeholder={t("auto.optional_for_example")}
            className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-1.5 text-sm text-gray-200 placeholder-gray-500 outline-none focus:border-indigo-500"
          />
        </div>
        ...
        <div>
          <div className="mb-1 flex items-center justify-between">
            <label className="block text-xs font-medium text-gray-400">
              {t("auto.reference_picture")} <span className="text-gray-600">{t("auto.optional")}</span>
            </label>
            {referenceFile && (
          ...
          {referencePreview ? (
            <div className="relative overflow-hidden rounded-lg border border-gray-700 bg-gray-800">
              <img
                ...
                alt={t("auto.character_reference_")}
