...
```bash
# 提交所有剧集/缺失分镜图到生成队列（自动从 project.json 获取剧本文件）
python .claude/skills/generate-storyboard/scripts/generate_storyboard.py

# 为单个场景重新生成（可指定具体剧本）
python .claude/skills/generate-storyboard/scripts/generate_storyboard.py episodes/E1.json --scene E1S05

# 为多个场景重新生成
python .claude/skills/generate-storyboard/scripts/generate_storyboard.py episodes/E1.json --scene-ids E1S01 E1S02
```
